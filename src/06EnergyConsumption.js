...
function totalEnergyConsumption(data, {
    ...
    const y2Scale = d3.scaleLinear()
        .domain([0, d3.max(data, d => Math.max(d.能源消费总量))]).nice()
        .range([height - marginBottom, marginTop]);
    ...
    svg.selectAll(".bar")
        ...
        .attr("y", function (d) { return y2Scale(d.能源消费总量); })
        .attr("height", function (d) { return y2Scale(0) - y2Scale(d.能源消费总量); })
        .attr("fill", "#A4B6CA");
            ...
            if (intersectedPoint) {
                ...
                if (intersectedPoint.年份 !== currentYear) {
                    ...
                    svg.append("text")
                        ...
                        .attr("y", 30)
                        .text(`${intersectedPoint.年份} 年 能源消费总量：${intersectedPoint.能源消费总量}`).style("font-size", "12px");
